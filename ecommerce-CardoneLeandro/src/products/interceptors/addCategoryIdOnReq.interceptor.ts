...
export class AddCategoryIdonRequestInterceptor implements NestInterceptor {
    ...
    async intercept(context: ExecutionContext, next: CallHandler<any>):Promise<Observable<any>> {
        ...
        console.log("CARDONE =========> addCategoryIdonRequestInterceptor, productCategory", productCategory)
        if(!productCategory) {
            throw new BadRequestException('category is required')
        }
        const category = await this.findCategoryByName(productCategory)
        console.log("CARDONE =========> addCategoryIdonRequestInterceptor, category", category)
        if (!category) {
            throw new BadRequestException('Category not found or does not exist');
        }
        delete request.body.category
        request.body.category_id = category.id;
    console.log("CARDONE =========> addCategoryIdonRequestInterceptor, request", request.body)
    return next.handle()
}
    ...
    async findCategoryByName(name: string) {
        try {
            ...
            console.log("CARDONE =========> addCategoryIdonRequestInterceptor catch", error)
